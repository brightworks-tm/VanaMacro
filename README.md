...
### マクロ内の定型文について
- マクロ内の定型文（`<<定型文>>`）は、UI言語とは**独立**しています。
- 定型文の言語は、「FFXIから取り込み」を実行した際のFFXIクライアント言語に依存します。
  - 日本語クライアントから取り込み → 日本語の定型文
  - 英語クライアントから取り込み → 英語の定型文
...
- `autotrans_data/` … 定型文・アイテム辞書（`bin/item`, `bin/text`, `bin/tree` 等）と Lua 元データ。
...
- **オートトランス展開・補完**: `autotrans_data/bin`（なければ `res`）の辞書を参照。

## エディタ支援機能
- シンタックスハイライト: コマンド、ターゲット、定型文括弧（<< / >>）、引用符内文字列、コメントを基本色で表示。
- バトル関連語の色分け: ジョブアビリティ > 魔法 > ウェポンスキル > 条件付き JA > ペットコマンドの優先順位で重複を避けつつハイライトします。魔法には BardSong/忍術/召喚/青魔/風水に加え、定型文「ウタ」リストも取り込み、敵専用魔法（Curse など）は自動除外しています。
- コマンド辞書: `/` で始まる語は FFXI 正式コマンドのみを照合し、`autotrans_data/res/auto_translates.lua` の「テキストコマンド」カテゴリから自動生成されます。
...
- **辞書の再生成**: `autotrans_data/tools/sync_auto_tables.py` を実行すると、`res/*.lua` から `autotrans.db` を生成します。
- **FFXI アップデート後の更新手順**:
...
- **多言語対応**: データベースには日本語 (ja) と英語 (en) の両方のデータが含まれており、将来的な英語クライアント対応の基盤となっています。
...
- **Book名の反映**: VanaMacro 上で Book 名を変更しても、**FFXI ゲーム内には反映されません**（FFXI の仕様上の制限）。Book 名の変更はゲーム内で行ってください。マクロの内容（テキスト）は正常に反映されます。
- FFXI 本体には Set 名の概念がないため、Set 名はツール内の見出し用と割り切ってください。
...
5. **FFXI への反映**: エクスポート完了後、画面下部の「FFXI USER フォルダへコピー（上書き）」ボタンを押すことで、生成したファイルを FFXI USER フォルダへコピーできます。**このボタン操作でのみ FFXI 側への上書きが行われます。**
...
※ 実際のスクリーンショットは `docs/screenshots/` などに配置して、この節の箇条書きを画像リンクに置き換えてください。
